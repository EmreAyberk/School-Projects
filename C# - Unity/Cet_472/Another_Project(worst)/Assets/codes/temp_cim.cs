...
public class temp_cim : MonoBehaviour {
    ...
    public void cim()
    {
        if (cimlendir)
        {
            if (carbon1 % 2 == 0)
            {
                plant.GetComponent<Animator>().Play("Temp_1");
            }
            if (carbon2 % 2 == 0)
            {
                plant1.GetComponent<Animator>().Play("Temp_2");
            }
            notepad_p5.SetActive(true);
            complete = true;
        }
    }
    public void done()
